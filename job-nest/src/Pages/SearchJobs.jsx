...
export default function SearchJobs() {
    ...
    const fetchListings = async (searchTerm = "", sort = "createdAt", order = "desc", startIndex = 0) => {
        setLoading(true)
        setShowmore(false)
    ...
    useEffect(() => {
        ...
        fetchListings(searchTermFromUrl || "", sortFromUrl || "createdAt", orderFromUrl || "desc")
    },[location.search])

    const handleChange = (e) => {
        if(e.target.id === "searchTerm") {
            setsearchData({
            ...searchData, searchTerm: e.target.value,
        })
        }
    ...
    const onshowMoreClick = async () => {
        ...
        fetchListings(searchData.searchTerm, searchData.sort, searchData.order, startIndex)
    }
